...
router.put('/attendance/:id', roleMiddleware('Super Admin', 'Admin', 'Staff'), (req, res) => {
  ...
  db.prepare('UPDATE attendance SET check_in=?,check_out=?,status=?,late_minutes=?,penalty_amount=?,check_out_lat=?,check_out_lng=?,check_out_photo=?,check_out_location_id=?,check_out_distance=? WHERE id=?')
    .run(req.body.check_in, req.body.check_out, req.body.status, lateMinutes, penaltyAmount, req.body.check_out_lat || null, req.body.check_out_lng || null, req.body.check_out_photo || null, req.body.check_out_location_id || null, req.body.check_out_distance || null, req.params.id);
