  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-6">
        ...
        <button onClick={() => { setShowForm(true); setEditId(null); setForm({ number: `INV-${new Date().getFullYear()}-${String(data.length+1).padStart(3,'0')}`, client_id: '', project_id: '', date: new Date().toISOString().slice(0,10), due_date: '', subtotal: '', tax: '', status: 'Draft', items_json: '[]' }); }} className="bg-blue-500 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-600 flex items-center gap-2"><Plus size={16} /> Tambah Invoice</button>
      </div>
            ...
            <tbody className="divide-y divide-gray-100">
              {data.map(inv => (
                <tr key={inv.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-gray-600">{custName(inv.client_id)}</td>
                  <td className="px-4 py-3 text-gray-600">{inv.date}</td>
                  <td className="px-4 py-3 text-gray-600">{inv.due_date}</td>
                  <td className="px-4 py-3 text-gray-600">{fmt(inv.total)}</td>
                  <td className="px-4 py-3"><StatusBadge status={inv.status} /></td>
                  <td className="px-4 py-3"><div className="flex gap-2"><button onClick={() => setShowDetail(inv)} className="text-gray-500 hover:text-gray-700"><Eye size={16} /></button><button onClick={() => handleEdit(inv)} className="text-blue-500 hover:text-blue-700"><Edit2 size={16} /></button><button onClick={() => handleDelete(inv.id)} className="text-red-500 hover:text-red-700"><Trash2 size={16} /></button></div></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {showDetail && (
        <div className="fixed inset-0 bg-black/40 flex items-end sm:items-center justify-center z-50 p-0 sm:p-4">
          <div className="bg-white rounded-t-2xl sm:rounded-xl p-5 sm:p-6 w-full sm:max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4"><h3 className="font-semibold text-lg">Detail Invoice</h3><button onClick={() => setShowDetail(null)}><X size={20} /></button></div>
