...
import { TrendingUp, TrendingDown, DollarSign, Building2, ArrowUpCircle, ArrowDownCircle, Calendar, FileDown, Printer } from 'lucide-react';
import { exportReportPDF } from '../../lib/exportUtils';
...
export default function Reports() {
  ...
  const handleExportPDF = () => {
    if (tab === 'pl') {
      ...
      exportReportPDF('Neraca', data, ['Deskripsi', 'Jumlah'],
        [['Kas & Bank', fmt(data.cashBalance || 0)], ['Piutang', fmt(data.receivables || 0)], ['Aset Tetap', fmt(data.fixedAssets || 0)], ['Total Aset', fmt(data.totalAssets || 0)], ['Hutang', fmt(data.payables || 0)], ['Modal', fmt(data.equity || 0)], ['Laba Ditahan', fmt(profit)]]);
    }
  };
    ...
    <div>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-6">
        ...
        <div className="flex items-center gap-2 no-print">
          ...
          <button onClick={handleExportPDF} className="px-3 py-2 bg-green-600 text-white rounded-lg text-sm flex items-center gap-1"><FileDown size={14} /> PDF</button>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {Object.entries(months).map(([m, v]) => {
                ...
                return (<tr key={m} className="hover:bg-gray-50"><td className="px-4 py-3 font-medium">{m}</td><td className="px-4 py-3 text-right text-green-600">+{fmt(v.income)}</td><td className="px-4 py-3 text-right text-red-600">-{fmt(v.expense)}</td><td className={`px-4 py-3 text-right font-medium ${net >= 0 ? 'text-green-600' : 'text-red-600'}`}>{fmt(net)}</td></tr>);
              })}
