...
import { Plus, ArrowLeftRight, Search, X, Building2 } from 'lucide-react';
...
export default function Banking() {
  ...
  const [form, setForm] = useState({ type: 'income', account_id: '', amount: '', date: new Date().toISOString().slice(0, 10), description: '', reference: '', category: '' });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    await api.post('/banking/transactions', { ...form, amount: Number(form.amount), account_id: Number(form.account_id) });
    setShowForm(false); setForm({ type: 'income', account_id: '', amount: '', date: new Date().toISOString().slice(0, 10), description: '', reference: '', category: '' }); load();
  };
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-6">
        <h2 className="text-xl font-bold text-gray-800">Transaksi Banking</h2>
        <div className="flex gap-2">
          <button onClick={() => setShowTransfer(true)} className="bg-purple-500 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-purple-600 flex items-center gap-2"><ArrowLeftRight size={16} /> Transfer Internal</button>
